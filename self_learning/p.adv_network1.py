...
# Lines
hv_lines = pd.read_csv("pandapower-modelling/tutorials/example_advanced/hv_lines.csv", sep=";", header=0, decimal=",")
print(hv_lines)
...
for _, hv_line in hv_lines.iterrows():
    ...
    # ⚠️ Cast length and parallel explicitly to avoid dtype issues
    pp.create_line(net, from_bus, to_bus, length_km=float(hv_line.length), std_type=hv_line.std_type,
                   name=hv_line.line_name, parallel=int(hv_line.parallel))
...
pp.create_transformer_from_parameters(net, hv_bus, lv_bus, sn_mva=300, vn_hv_kv=380, vn_lv_kv=110, vkr_percent=0.6,
...
# Switches
hv_bus_sw = pd.read_csv("pandapower-modelling/tutorials/example_advanced/hv_bus_sw.csv", sep=";", header=0, decimal=",")
print(hv_bus_sw)
...
for _, switch in hv_bus_sw.iterrows():
    ...
    pp.create_switch(net, from_bus, to_bus,
                     et=switch.et, closed=switch.closed, 
                     type=switch.type, name=switch.bus_name)
...
for _, line in hv_ls.iterrows():
    pp.create_switch(net, line.from_bus,
                     line.name, et='l', closed=True, type='LBS',
                     name='Switch %s - %s' % (net.bus.name.at[line.from_bus], line['name']))
    pp.create_switch(net, line.to_bus, line.name, 
                     et='l', closed=True, type='LBS', 
...
pp.create_ext_grid(net, pp.get_element_index(net, "bus", 'Double Busbar 1'), vm_pu=1.03, va_degree=0,
                   name='External grid', s_sc_max_mva=10000, rx_max=0.1, rx_min=0.1)
print(net.ext_grid)

# Loads
hv_loads = pd.read_csv("pandapower-modelling/tutorials/example_advanced/hv_loads.csv", sep=";", header=0, decimal=",")
print(hv_loads)

for _, load in hv_loads.iterrows():
    bus_idx = pp.get_element_index(net, "bus", load.bus)
    pp.create_load(net, bus_idx, p_mw=float(load.p), q_mvar=float(load.q), name=load.load_name)
...
# Static Generator
pp.create_sgen(net, pp.get_element_index(net, "bus", "Bus SB 5"), vm_pu=1.03,
               p_mw=4, q_mvar=4, sn_mva=4, type="WP", name="Wind Park")
print(net.sgen)
...
# Lines
mv_lines = pd.read_csv("pandapower-modelling/tutorials/example_advanced/mv_lines.csv", sep=";", header=0, decimal=",")
for _, mv_line in mv_lines.iterrows():
    ...
    pp.create_line(net, from_bus, to_bus, length_km=float(mv_line.length), std_type=mv_line.std_type, name=mv_line.line_name)

print(net.line[net.line.from_bus.isin(mv_buses.index)])
...
pp.create_transformer3w_from_parameters(net, hv_bus, mv_bus, lv_bus, vn_hv_kv=110, vn_mv_kv=20, vn_lv_kv=10,
                                        ...
                                        shift_mv_degree=30, shift_lv_degree=30, tap_side="hv", tap_neutral=0,
                                        tap_min=-8, tap_max=8, tap_step_percent=1.25, tap_pos=0,
                                        name='HV-MV-MV-Trafo', tap_changer_type="Ratio")
...
print(net.switch[net.switch.bus.isin(mv_buses)])

# Loads
mv_loads = pd.read_csv("pandapower-modelling/tutorials/example_advanced/mv_loads.csv", sep=";", header=0, decimal=",")
for _, load in mv_loads.iterrows():
    bus_idx = pp.get_element_index(net, "bus", load.bus)
    pp.create_load(net, bus_idx, p_mw=float(load.p), q_mvar=float(load.q), name=load.load_name)
# show only medium voltage loads
print(net.load[net.load.bus.isin(mv_buses)])

# Static Generators
mv_sgens = pd.read_csv("pandapower-modelling/tutorials/example_advanced/mv_sgens.csv", sep=";", header=0, decimal=",")
for _, sgen in mv_sgens.iterrows():
    bus_idx = pp.get_element_index(net, "bus", sgen.bus)
    pp.create_sgen(net, bus_idx, p_mw=float(sgen.p), q_mvar=float(sgen.q), sn_mva=float(sgen.sn), name=sgen.sgen_name)
# show only medium voltage static generators 
print(net.sgen[net.sgen.bus.isin(mv_buses)])
...
# Busses
pp.create_bus(net, vn_kv=0.4, name="Bus LV0", type="n")
for i in range(1, 6):
    pp.create_bus(net, vn_kv=0.4, name="Bus LV1.%s" % i, type="m")
for i in range(1, 5):
    pp.create_bus(net, vn_kv=0.4, name="Bus LV2.%s" % i, type="m")
pp.create_bus(net, vn_kv=0.4, name="Bus LV2.2.1", type="m")
pp.create_bus(net, vn_kv=0.4, name="Bus LV2.2.2", type="n")

# Show only low voltage busses
lv_buses = net.bus[net.bus.vn_kv == 0.4]
print(lv_buses)


#Lines
lv_lines = pd.read_csv("pandapower-modelling/tutorials/example_advanced/lv_lines.csv", sep=";", header=0, decimal=",")
for _, lv_line in lv_lines.iterrows():
    ...
    pp.create_line(net, from_bus, to_bus, length_km=float(lv_line.length), std_type=lv_line.std_type, name=lv_line.line_name)

print(net.line[net.line.from_bus.isin(lv_buses.index)])
...
pp.create_transformer_from_parameters(net, hv_bus, lv_bus, sn_mva=.4, vn_hv_kv=10, vn_lv_kv=0.4, vkr_percent=1.325,
                                      vk_percent=4, pfe_kw=0.95, i0_percent=0.2375, tap_side="hv", tap_neutral=0, 
                                      tap_min=-2, tap_max=2, tap_step_percent=2.5, tap_changer_type="Ratio", tp_pos=0,
                                      shift_degree=150, name='MV-LV-Trafo')

print(net.trafo[net.trafo.lv_bus.isin(lv_buses.index)])
...
pp.create_switch(net, pp.get_element_index(net, "bus", 'Bus MV4'), 
                 ...
                 et='t', closed=True, type='LBS', name='Switch MV4 - MV-LV-Trafo')
pp.create_switch(net, pp.get_element_index(net, "bus", 'Bus LV0'), 
                 pp.get_element_index(net, "trafo", 'MV-LV-Trafo'), 
                 et='t', closed=True, type='LBS', name='Switch LV0 - MV-LV-Trafo')
...
#Loads
lv_loads = pd.read_csv("pandapower-modelling/tutorials/example_advanced/lv_loads.csv", sep=";", header=0, decimal=",")
for _, load in mv_loads.iterrows():
    bus_idx = pp.get_element_index(net, "bus", load.bus)
    pp.create_load(net, bus_idx, p_mw=float(load.p), q_mvar=float(load.q), name=load.load_name)
# show only medium voltage loads
...
#Static Generators
lv_sgens = pd.read_csv("pandapower-modelling/tutorials/example_advanced/lv_sgens.csv", sep=";", header=0, decimal=",")
for _, sgen in lv_sgens.iterrows():
    bus_idx = pp.get_element_index(net, "bus", sgen.bus)
    pp.create_sgen(net, bus_idx, p_mw=float(sgen.p), q_mvar=float(sgen.q), sn_mva=float(sgen.sn), name=sgen.sgen_name)
# show only medium voltage static generators 
print(net.sgen[net.sgen.bus.isin(lv_buses.index)])
